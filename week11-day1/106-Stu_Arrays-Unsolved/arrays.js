...
const names = princesses.map((item) => {
  return item.name;
});
// let names = [];
// for (let i = 0; i < princesses.length; i++) {
...
const mNames = princesses.filter((item) => item.name.includes("M"));
// let mNames = [];
...
const sum = princesses.reduce((total, item) => total + item.age, 0);
// let sum = 0;
// for (let i = 0; i < princesses.length; i++) {
